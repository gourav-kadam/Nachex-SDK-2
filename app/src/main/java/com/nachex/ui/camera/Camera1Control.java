...
public class Camera1Control implements ICameraControl {
    ...
    /*
     * Local quality control scan mode
     */
    ...
    private void onRequestDetect(byte[] data) {
        ...
        try {
            ...
            img.compressToJpeg(new Rect(0, 0, optSize.width, optSize.height), 100, os);
        ...
        if (status == 0) {
            ...
            // If the memory overflows, the current operation will be canceled
        } finally {
            try {
                os.close();
            } catch (Throwable e) {
    ...
    public void stop() {
        if (camera != null) {
            ...
            stopPreview();
            // Avoid synchronous code, in order to set null first and then release
            Camera tempC = camera;
            camera = null;
            tempC.release();
            camera = null;
    ...
    Camera.PreviewCallback previewCallback = new Camera.PreviewCallback() {
        ...
        public void onPreviewFrame(final byte[] data, Camera camera) {
            // A successful scan prevents opening new threads for processing
            if (abortingScan.get()) {
                return;
            }
            // throttling
            ...
            // In some models and projects, the data of some frames does not conform to the nv21 format and needs to be filtered, otherwise the subsequent processing will cause a crash
                ...
                synchronized (Camera1Control.this) {
                    if (camera != null && !takingPicture.get()) {
                        try {
                            ...
                            // startPreview is implemented asynchronously and may autofocus failß in the first few calls on some machines
                        }
                    }
                }
            }
    ...
    /**
     * Some cameras don't match the perfect ratio. for example. Our layout is 4:3. Preview only 16:9
     * If the picture is displayed directly, it will be stretched and deformed. When zoomed in, there are black borders. So our strategy
     * Yes, scale up. This way part of the preview will go off the screen. Crop after taking pictures.
     */
    private class PreviewView extends FrameLayout {
        ...
        private void relayout(int w, int h) {
            ...
            int height = h;
            if (w < h) {
                // Vertical mode with fixed height.
                height = (int) (width * ratio);
            } else {
                // Horizontal mode with fixed width.
